package com.keensense.sdk.sys.utils;
...
public class DbPropUtil {
	...
	public static void start(final String name) {
		ExecutorService service = ThreadUtil.newSingleThreadExecutor();
		MODULE_NAME = name;
		reload(MODULE_NAME);
		service.execute(() -> {
			try {
				while (true) {
					Thread.sleep(10000L);
					DbPropUtil.reload(MODULE_NAME);
				}
			} catch (InterruptedException var2) {
				log.error("InterruptedException", var2);
