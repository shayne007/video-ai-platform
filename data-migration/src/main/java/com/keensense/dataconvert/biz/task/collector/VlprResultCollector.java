...
import com.keensense.dataconvert.biz.task.callable.VlprResultCollectCall;
 ...
 * @Package：com.keensense.dataconvert.biz.task.collector
 * @Description： <p> VlprResultCollector </p>
 * @Author： - Jason
 * @CreatTime：2019/7/25 - 16:59
 * @Modify By：
 * @ModifyTime： 2019/7/25
 * @Modify marker：
 */
@Component
public class VlprResultCollector {
     ...
     * name 线程区分
     */
    public static String COLLECTOR_NAME = "convert-data-vlpr";
     ...
     * 开始启动
     */
    public void startCollector(List<VlprResult> resultsList, String tableYmd) {
        ...
        if (threadNum > 0) {
             ...
             * callable
             */
            VlprResultCollectCall resultCollectCall;
            for (VlprResult vlprResult : resultsList) {
                resultCollectCall = new VlprResultCollectCall(vlprResult,tableYmd);
                /**
            ...
            for (FutureTask<TaskResultInfo> futureTask : taskList) {
                try {
                    ...
                    if (taskResultInfo.getCollectCallCode() != ResultEnum.BIZ_DEAL_SUCCESS.getResultCode()){
                        ...
                        redisService.lpush(RedisConstant.REDIS_MYSQL_VLPR_TO_ES_ERROR_LIST_KEY.concat(collectTableYmdStr),
