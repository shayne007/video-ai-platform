...
import com.keensense.densecrowd.mapper.sys.SysModuleMapper;
...
public class SysModuleServiceImpl extends ServiceImpl<SysModuleMapper, SysModule> implements ISysModuleService {
    ...
    @Override
    public List<SysModule> selectMenuLsitByLevel(Long level) {
        Set<Long> menuIdSet = new HashSet<>();
        List<SysModule> sysPermissions = baseMapper.selectList(new QueryWrapper<SysModule>().eq("module_level", level));
        for (SysModule perm : sysPermissions) {
            menuIdSet.add(perm.getModuleId());
        }
        List<Long> menuIdList = new ArrayList<>(menuIdSet);
        //不以角色过滤菜单显示
        return getAllMenuList(0, menuIdList);
